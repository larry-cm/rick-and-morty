...
export default function Menu (){
    ...
    return(
        ...
        onClick={handleClick}
        class={` font-medium relative group transition-all duration-200 w-max h-max p-2 ${vMenu ?"overflow-visible":"overflow-hidden"}  flex flex-row items-center justify-center bg-gray-800 gap-2 rounded-lg border border-white/25 shadow-md text-white`}
>

  <span > Menu </span>
  <svg
    class={` ${vAni ? "rotate-180":"rotate-90"} transition-transform duration-500 size-[22px]`}
  ...
  {/* menu desplegable */}
  <div
    class={`absolute shadow-lg -bottom-40 left-0 w-full h-max p-2 bg-gray-800 border border-white/25 rounded-lg flex flex-col gap-2 ${vAni ? "animate-fade-out":"animate-fade-in"} duration-500`}
  >
    <span
